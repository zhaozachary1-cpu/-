(() => {
  ...
  styles.textContent = `
    ...
    .calendar-panel{padding:14px;overflow:hidden}.calendar-header{display:flex;align-items:center;gap:8px;margin-bottom:11px}.calendar-heading{margin-right:auto}.calendar-heading .eyebrow{margin-bottom:2px}.calendar-heading h2{font-size:18px;margin:0;letter-spacing:-.3px}.calendar-control{width:29px;height:29px;border:1px solid var(--line);border-radius:9px;background:#0d1630;color:#cdd9f6;font-size:18px;line-height:1;cursor:pointer;transition:transform .16s,background .16s}.calendar-control:hover{background:#24345e;transform:translateY(-1px)}.calendar-today{border:1px solid #526ae0;color:#cdd8ff;background:#5165d51e;border-radius:8px;padding:7px 10px;font-size:10px;font-weight:700;cursor:pointer}.calendar-weekdays,.calendar-grid{display:grid;grid-template-columns:repeat(7,minmax(0,1fr))}.calendar-weekdays{border:1px solid var(--line);border-bottom:0;border-radius:12px 12px 0 0;overflow:hidden}.calendar-weekdays span{padding:7px 10px;color:#8697bf;font-size:9px;text-align:right;border-right:1px solid var(--line)}.calendar-weekdays span:last-child{border-right:0}.calendar-grid{border:1px solid var(--line);border-radius:0 0 12px 12px;overflow:hidden}.calendar-day{height:76px;padding:5px 6px;border-right:1px solid var(--line);border-bottom:1px solid var(--line);background:#09112682;transition:background .16s}.calendar-day:nth-child(7n){border-right:0}.calendar-day:nth-last-child(-n+7){border-bottom:0}.calendar-day.outside{background:#060c1c80}.calendar-day.today{background:linear-gradient(145deg,#202c5d88,#101831b8)}.calendar-date{display:flex;justify-content:flex-end;align-items:center;height:17px;font-size:10px;color:#a7b4d0}.calendar-day.outside .calendar-date{color:#54627f}.calendar-day.today .calendar-date span{display:grid;place-items:center;width:18px;height:18px;border-radius:50%;background:#6675ef;color:#fff;font-weight:800}.calendar-event{display:block;width:100%;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;margin-top:3px;border:0;border-left:3px solid var(--cyan);border-radius:4px;background:#29396d9c;color:#e7ecff;text-align:left;padding:4px 5px;font-size:9px;cursor:pointer;transition:transform .16s,filter .16s}.calendar-event:hover{transform:translateX(2px);filter:brightness(1.15)}.calendar-event.pending{border-left-color:var(--orange);background:#6f482733}.calendar-event.waiting{border-left-color:#b394ff;background:#5f488533}.calendar-event.offer{border-left-color:var(--green);background:#27654d33}.calendar-event.failed{border-left-color:var(--red);background:#71344433}.calendar-more{display:block;margin-top:3px;color:#9cace8;font-size:9px;line-height:1.2}.calendar-legend{display:flex;align-items:center;gap:13px;margin-top:10px;color:#91a0c0;font-size:9px}.calendar-legend span:before{content:'';display:inline-block;width:6px;height:6px;border-radius:50%;margin-right:4px;background:currentColor}.calendar-legend .pending{color:var(--orange)}.calendar-legend .waiting{color:#b394ff}.calendar-legend .offer{color:var(--green)}.calendar-legend .failed{color:var(--red)}
  `;
  ...
  const render = () => {
    ...
    const days = Array.from({ length: calendarDays }, (_, index) => {
      ...
      const events = dayEvents.slice(0, 1).map((record) => `<button type="button" class="calendar-event ${map[record.status]}" data-record-id="${record.id}" title="${esc(record.company)} · ${esc(record.role)}">${formatTime(record.date)} ${esc(record.company).slice(0, 9)}</button>`).join('');
