(() => {
  ...
  if (form) {
    ...
    profile.innerHTML = `<div class="company-profile-heading"><div><span>QCC COMPANY PROFILE</span><strong>企查查企业画像</strong></div><em>查询后可手动编辑</em></div><div class="company-profile-grid">${profileFields.map(([name, label]) => `<label>${label}<input name="${name}" placeholder="查询后自动填充"></label>`).join('')}</div>`;
  ...
  draw = () => {
    ...
    companyFacts.forEach(([name, label]) => {
